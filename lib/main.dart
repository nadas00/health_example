  ...
  Future fetchStepData() async {
    ...
    if (requested) {
      try {
        ...
        List<HealthDataPoint> dataPoint = await health.getHealthDataFromTypes(now, midnight, [HealthDataType.STEPS]);
      } catch (error) {
    ...
    return ListView.builder(
        ...
        itemBuilder: (_, index) {
          ...
          if (p.value is WorkoutHealthValue) {
            return ListTile(
              title: Text("${p.typeString}: ${(p.value as WorkoutHealthValue).totalEnergyBurned} ${(p.value as WorkoutHealthValue).totalEnergyBurnedUnit?.typeToString()}"),
